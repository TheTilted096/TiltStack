...
static py::array_t<T> asArray2D(T *data, ssize_t rows, ssize_t cols,
    ...
    return py::array_t<T>(
        ...
        std::vector<ssize_t>{cols * stride, stride},
        data, base);
}
...
PYBIND11_MODULE(deepcfr, m) {
    ...
    py::class_<Scheduler>(m, "Scheduler")
        ...
        .def("input_data", [](py::object self) {
            auto &s = self.cast<Scheduler &>();
            return infoSetArray(
                reinterpret_cast<uint8_t *>(s.inputData()),
                s.batchSize(), self);
        })
        .def("output_data", [](py::object self) {
            auto &s = self.cast<Scheduler &>();
            return asArray2D<float>(reinterpret_cast<float *>(s.outputData()),
                                    s.batchSize(), (ssize_t)NUM_ACTIONS, self);
        })
        .def("submit_batch", [](Scheduler &s) {
        ...
        .def("advantage_input_data", [](py::object self) {
            auto &s = self.cast<Scheduler &>();
            return infoSetArray(
                reinterpret_cast<uint8_t *>(s.advantageInputData()),
                s.advantageSize(), self);
        })
        .def("advantage_output_data", [](py::object self) {
            auto &s = self.cast<Scheduler &>();
            return asArray2D<float>(reinterpret_cast<float *>(s.advantageOutputData()),
        ...
        .def("policy_input_data", [](py::object self) {
            auto &s = self.cast<Scheduler &>();
            return infoSetArray(
                reinterpret_cast<uint8_t *>(s.policyInputData()),
                s.policySize(), self);
        })
        .def("policy_output_data", [](py::object self) {
            auto &s = self.cast<Scheduler &>();
            return asArray2D<float>(reinterpret_cast<float *>(s.policyOutputData()),
    ...
    py::class_<Orchestrator>(m, "Orchestrator")
        ...
        .def("pop", [](Orchestrator &o) -> Scheduler * {
            py::gil_scoped_release release;
            return o.pop();
