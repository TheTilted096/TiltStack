...
static void primeGlobals() {
    ...
    for (int i = 0; i < 9; i++) {
        int j = i + static_cast<int>(tmp.next() % static_cast<uint64_t>(52 - i));
    ...
    for (int p = 0; p < 2; p++) {
        uint8_t cards[7] = {
            deck[p*2], deck[p*2+1], deck[4], deck[5], deck[6], deck[7], deck[8]
        };
        hand_index_t indices[4];
        hand_index_all(&idx, cards, indices);
        for (int r = 0; r < 4; r++) {
            auto i = static_cast<std::size_t>(indices[r]);
            if (i >= gEHS[r].size())    gEHS[r].resize(i + 1, 0);
            if (i >= gLabels[r].size()) gLabels[r].resize(i + 1, 0);
        }
    }
...
#define MAKE_GAME() primeGlobals(); rng.seed(TEST_SEED); CFRGame g
...
TEST(CFRGame, ChipsConserved) {
    MAKE_GAME();
    auto chips = [&]{ return g.stacks[0] + g.stacks[1] + g.history[g.ply].pot; };
...
// makeMove then unmakeMove must leave stacks, pot, round, and betHist unchanged.
...
// CALL mid-street when a stack equals toCall is an all-in showdown (terminal 2).
// Completing the action on the river is a showdown (terminal 2).
TEST(CFRGame, TerminalDetection) {
    MAKE_GAME();
    EXPECT_EQ(g.isTerminalState(Action::CHECK), 1); // immediate fold

    rng.seed(TEST_SEED); CFRGame g2;
    g2.makeMove(Action::ALLIN);
    EXPECT_EQ(g2.isTerminalState(Action::CALL), 2); // all-in call

    rng.seed(TEST_SEED); CFRGame g3;
    g3.makeMove(Action::CALL);  g3.makeMove(Action::CHECK); // → flop
    g3.makeMove(Action::CHECK); g3.makeMove(Action::CHECK); // → turn
    g3.makeMove(Action::CHECK); g3.makeMove(Action::CHECK); // → river
    g3.makeMove(Action::CHECK);                              // first river check
    EXPECT_EQ(g3.isTerminalState(Action::CHECK), 2);
}

// All five actions available in the initial state; CALL absent when toCall == 0.
TEST(CFRGame, GenerateActions) {
    ...
    for (int i = 0; i < n; i++) EXPECT_NE(al[i], Action::CALL);
}
...
TEST(CFRGame, GetInfo) {
    ...
    EXPECT_FLOAT_EQ(info.myStack,  static_cast<float>(STARTING_STACK - SMALL_BLIND) / eff);
    EXPECT_FLOAT_EQ(info.oppStack, static_cast<float>(STARTING_STACK - BIG_BLIND)   / eff);
    EXPECT_FLOAT_EQ(info.potSize,  static_cast<float>(SMALL_BLIND + BIG_BLIND)      / eff);
