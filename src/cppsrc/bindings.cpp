/*
    ...
    Exposes:
      ...
      - RiverExpander:  computes 169-dim equity vectors + EHS + multiplicities
   for river states
      - TurnIndexer:    maps turn state indices to card strings
      - TurnExpander:   computes 256-dim histograms, per-state EHS, and
   multiplicities
      - FlopIndexer:    maps flop state indices to card strings; forward-indexes
   cards
      - FlopExpander:   computes 256-dim histograms, per-state EHS, and
   multiplicities
*/
...
class RiverIndexer {
  ...
  public:
    ...
    batch_unindex(const std::vector<uint64_t> &indices) const {
...
class PreflopIndexer {
  ...
  public:
    ...
    uint64_t
    index(py::array_t<uint8_t, py::array::c_style | py::array::forcecast> cards)
        const {
        auto buf = cards.request();
        if (buf.size != 2)
            throw std::invalid_argument(
                "PreflopIndexer.index: expected array of 2 cards");
...
class FlopIndexer {
  ...
  public:
    ...
    index(py::array_t<uint8_t, py::array::c_style | py::array::forcecast> cards)
        ...
        if (buf.size != 5)
            throw std::invalid_argument(
                "FlopIndexer.index: expected array of 5 cards");
        return hand_index_last(&indexer_, static_cast<uint8_t *>(buf.ptr));
    }

    py::array_t<uint64_t> batch_index(
        py::array_t<uint8_t, py::array::c_style | py::array::forcecast> cards)
        const {
        auto buf = cards.request();
        if (buf.ndim != 2 || buf.shape[1] != 5)
            throw std::invalid_argument(
                "FlopIndexer.batch_index: expected (N, 5) uint8 array");
...
// Stream all river states in batches, calling callback(equity, ehs, mult) for
// each. equity is (B, 169) uint8; ehs is (B,) float32; mult is (B,) uint8.
static void py_expand_all_with_ehs_mult(const RiverExpander &exp,
    ...
    for (uint64_t start = 0; start < total; start += batch_size) {
        const int actual =
            static_cast<int>(std::min<uint64_t>(batch_size, total - start));
        ...
        {
            py::gil_scoped_release release;
            exp.compute_range_ehs_mult(start, actual, equity.mutable_data(),
                                       ehs.mutable_data(), mult.mutable_data());
        }
...
static void py_turn_expand_all_with_ehs_mult(const TurnExpander &exp,
    ...
    for (uint64_t start = 0; start < total; start += batch_size) {
        ...
        {
            py::gil_scoped_release release;
            exp.compute_range_ehs_mult(start, actual, hist.mutable_data(),
                                       ehs.mutable_data(), mult.mutable_data());
        }
...
static py::tuple py_flop_compute_sample_ehs_mult(
    ...
    {
        py::gil_scoped_release release;
        exp.compute_rows_ehs_mult(idx_ptr, n, hist.mutable_data(),
                                  ehs.mutable_data(), mult.mutable_data());
    }
...
PYBIND11_MODULE(hand_indexer, m) {
    ...
    py::class_<PreflopIndexer>(m, "PreflopIndexer")
        ...
        .def("index", &PreflopIndexer::index, py::arg("cards"),
             "Map a 2-element uint8 array of card codes to a canonical preflop "
             "index.")
        .def("unindex", &PreflopIndexer::unindex, py::arg("index"),
    ...
    py::class_<RiverIndexer>(m, "RiverIndexer")
        ...
        .def("unindex", &RiverIndexer::unindex, py::arg("index"),
             "Convert a river state index to a card string")
        .def("batch_unindex", &RiverIndexer::batch_unindex, py::arg("indices"),
    ...
    py::class_<RiverExpander>(m, "RiverExpander")
        ...
        .def("num_states", &RiverExpander::num_states,
             "Total number of canonical river states (2,428,287,420)")
        .def("compute_sample", &py_compute_sample, py::arg("indices"),
        ...
        .def("expand_all_with_ehs_mult", &py_expand_all_with_ehs_mult,
             py::arg("callback"), py::arg("batch_size") = 1000000,
             "Stream all river states in batches, yielding equity, EHS, and "
             "multiplicity.\n"
             "Calls callback(equity, ehs, mult) where equity is (B, 169) "
             "uint8,\n"
             "ehs is (B,) float32, and mult is (B,) uint8.\n"
             "GIL is released during C++ computation and reacquired for each "
             "callback.");
    ...
    py::class_<TurnIndexer>(m, "TurnIndexer")
        ...
        .def("unindex", &TurnIndexer::unindex, py::arg("index"),
             "Convert a turn state index to a card string")
        .def("batch_unindex", &TurnIndexer::batch_unindex, py::arg("indices"),
             "Convert multiple indices to card strings");

    py::class_<TurnExpander>(m, "TurnExpander")
        .def(py::init<std::string, std::string>(), py::arg("river_labels_path"),
             py::arg("river_ehs_fine_path"),
             "Load both river files into RAM and initialise turn/river hand "
             "indexers.\n"
        ...
        .def(
            "compute_sample", &py_turn_compute_sample, py::arg("indices"),
            "Compute wide-bucket histograms for the given turn state indices.\n"
            "indices: 1-D uint64 array  →  returns (n, 256) uint8 array.\n"
            "Each row is a count histogram over 256 wide buckets summing to "
            "46.")
        .def("expand_all_with_ehs_mult", &py_turn_expand_all_with_ehs_mult,
             py::arg("callback"), py::arg("batch_size") = 500000,
             "Stream all turn states in batches, yielding histograms, EHS, and "
             "multiplicities.\n"
    ...
    py::class_<FlopIndexer>(m, "FlopIndexer")
        ...
        .def("index", &FlopIndexer::index, py::arg("cards"),
             "Map a 5-element uint8 array of card codes to a canonical flop "
             "index.")
        .def("unindex", &FlopIndexer::unindex, py::arg("index"),
             "Convert a flop state index to a card string")
        .def("batch_unindex", &FlopIndexer::batch_unindex, py::arg("indices"),
             "Convert multiple indices to card strings")
        .def("batch_index", &FlopIndexer::batch_index, py::arg("cards"),
             "Map an (N, 5) uint8 array of card-code rows to a (N,) uint64 "
             "array of canonical flop indices.");

    py::class_<FlopExpander>(m, "FlopExpander")
        .def(py::init<std::string, std::string>(), py::arg("turn_labels_path"),
             py::arg("turn_ehs_fine_path"),
             "Load both turn files into RAM and initialise flop/turn hand "
             "indexers.\n"
        ...
        .def(
            "compute_sample_ehs_mult", &py_flop_compute_sample_ehs_mult,
            py::arg("indices"),
            "Compute histograms, EHS, and multiplicities for the given flop "
            "state indices.\n"
            "indices: 1-D uint64 array  →  returns (hist, ehs, mult) tuple "
            "where\n"
