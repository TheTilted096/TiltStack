...
TurnExpander::TurnExpander(const std::string& river_labels_path,
    ...
    if (!river_ehs_fine_path.empty()) {
        ...
        if (!f)
            throw std::runtime_error("TurnExpander: cannot open " + river_ehs_fine_path);
        f.seekg(0, std::ios::end);
        const std::streamsize bytes = f.tellg();
        f.seekg(0, std::ios::beg);
        if (bytes % sizeof(uint16_t) != 0)
            throw std::runtime_error("TurnExpander: EHS file size not a multiple of 2");
        river_ehs_fine_.resize(static_cast<size_t>(bytes) / sizeof(uint16_t));
        f.read(reinterpret_cast<char*>(river_ehs_fine_.data()), bytes);
        if (!f)
            throw std::runtime_error("TurnExpander: read failed for " + river_ehs_fine_path);
    }
...
uint8_t TurnExpander::computeMult(hand_index_t turn_idx) const {
    ...
    const uint_fast32_t equal_mask = turn_indexer_.configuration_to_equal[round][cfg];
