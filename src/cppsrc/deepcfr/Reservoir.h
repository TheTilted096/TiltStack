...
//   advantageInputs.clear();  advantageOutputs.clear();
...
public:
    ...
    // inputBuf   : caller-owned buffer of capacity × sizeof(InfoSet) bytes (uint8)
    // targetBuf  : caller-owned buffer of capacity × NUM_ACTIONS floats
    // weightBuf  : caller-owned buffer of capacity int32s; nullptr if unused
    Reservoir(std::size_t capacity, int numThreads,
              uint8_t* inputBuf, float* targetBuf,
              int32_t* weightBuf = nullptr);
...
private:
    ...
    int         numThreads_;
    std::size_t sliceSize_;   // capacity_ / numThreads_ (last thread may be larger)
