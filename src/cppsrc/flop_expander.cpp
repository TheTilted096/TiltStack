...
FlopExpander::FlopExpander(const std::string& turn_labels_path,
...

    {
        ...
        if (!f)
            throw std::runtime_error("FlopExpander: cannot open " + turn_ehs_fine_path);
        f.seekg(0, std::ios::end);
        const std::streamsize bytes = f.tellg();
        f.seekg(0, std::ios::beg);
        if (bytes % sizeof(uint16_t) != 0)
            throw std::runtime_error("FlopExpander: EHS file size not a multiple of 2");
        turn_ehs_fine_.resize(static_cast<size_t>(bytes) / sizeof(uint16_t));
        f.read(reinterpret_cast<char*>(turn_ehs_fine_.data()), bytes);
        if (!f)
            throw std::runtime_error("FlopExpander: read failed for " + turn_ehs_fine_path);
    }
}
...
void FlopExpander::compute_rows_ehs_mult(const uint64_t* indices, size_t n,
    ...
    for (ptrdiff_t i = 0; i < (ptrdiff_t)n; i++)
        computeRowEhsMult(indices[i], row_out + i * DIMS,
...
uint8_t FlopExpander::computeMult(hand_index_t flop_idx) const {
    ...
    const uint_fast32_t equal_mask = flop_indexer_.configuration_to_equal[round][cfg];
