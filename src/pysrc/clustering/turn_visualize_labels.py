...
def plot_cluster_sizes(axes, counts, n):
    ...
    ax1.axvline(counts.mean(), color="#F44336", ls="--", lw=1.2,
                label=f"mean={counts.mean():,.0f}")
    ax1.axvline(np.median(counts), color="#FF9800", ls="--", lw=1.2,
...
def plot_representatives(centroids, counts, expected_ehs, proj, var_explained,
                         output_path):
    ...
    rep_labels = [f"P{p} (#{reps[i]}, E[EHS]={expected_ehs[reps[i]]:.2f})"
...
def main():
    ...
    if has_centroids:
        ...
        plot_representatives(centroids, counts, expected_ehs, proj,
