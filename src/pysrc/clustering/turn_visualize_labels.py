...
def load_counts_and_examples(
    path, k, cluster_ids, n_examples=5, seed=DEFAULT_EXAMPLE_SEED, chunk=10_000_000
):
...
def plot_hands(counts, expected_ehs, output_path, hand_examples):
    ...
    fig.suptitle(
        "Example Hands by E[EHS] Percentile (hole | board)",
        fontsize=14,
        fontweight="bold",
        y=0.98,
    )
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Visualize turn bucket label distribution and example hands."
    )
    parser.add_argument(
        "--seed",
        type=int,
