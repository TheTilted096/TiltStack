...
def train_centroids(sample: np.ndarray, k: int, niter: int, seed: int) -> np.ndarray:
    ...
    d = sample.shape[1]
    print(
        f"Training K={k:,} centroids, {niter} iterations on GPU "
        f"({sample.shape[0]:,} x {d} vectors)...",
        file=sys.stderr,
    )
    kmeans = faiss.Kmeans(
        d,
...
def assign_labels_and_ehs_fine_streaming(
    ...
    print(
        f"Assigning {total_states:,} vectors on GPU (batch_size={batch_size:,})...",
        file=sys.stderr,
    )
    t0 = time.time()
    total_assigned = 0
    ...
    with open(labels_path, "wb") as lf, open(ehs_fine_path, "wb") as ef:
        ...
        def process_batch(equity_uint8, ehs_float32, mult_uint8):
            ...
            ehs_sum += np.bincount(label_arr, weights=ehs_f64 * mult, minlength=k)
