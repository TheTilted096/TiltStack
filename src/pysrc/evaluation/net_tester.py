"""
net_tester.py — Duplicate match evaluation between TiltStack agents.
...
Usage:
    cd src
    python pysrc/evaluation/net_tester.py \\
        --net    ../checkpoints/policy0050.pt \\
        --br0    ../br_checkpoints/br_adv0_0040.pt \\
        --br1    ../br_checkpoints/br_adv1_0040.pt \\
        --pairs  10000 \\
        --device cpu
...
def main():
    ...
    parser.add_argument(
        "--net", required=True, help="Path to strategy network checkpoint (policy*.pt)"
    )
    parser.add_argument(
        "--br0", required=True, help="Path to best-response advantage network for P0"
    )
    parser.add_argument(
        "--br1", required=True, help="Path to best-response advantage network for P1"
    )
    parser.add_argument(
        "--pairs",
        type=int,
    ...
    print(
        f"  TiltStack edge as SB     : {sb_payoff / args.pairs * CHIPS_TO_MBB:+.2f} mBB/hand"
    )
    print(
        f"  TiltStack edge as BB     : {bb_payoff / args.pairs * CHIPS_TO_MBB:+.2f} mBB/hand"
    )
    print(
        f"  TiltStack overall edge   : {overall_mbb:+.2f} ± {ci95:.2f} mBB/hand  (95% CI)"
    )
