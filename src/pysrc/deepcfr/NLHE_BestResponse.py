...
def run_inference_loop(orch, hero: bool, adv_nets, policy_net, device):
    ...
    while done < orch.num_threads():
        ...
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            if len(hero_idx) > 0:
                out[hero_idx] = (
                    adv_nets[int(hero)](x_cont[hero_idx], buckets[hero_idx])
                    .float()
                    .cpu()
                    .numpy()
                )
            ...
            if len(villain_idx) > 0:
                logits = policy_net(x_cont[villain_idx], buckets[villain_idx])
...
def main():
    ...
    orch = deepcfr.Orchestrator(
        args.threads, adv_res[0]._cpp, adv_res[1]._cpp, seed=seed
    )
    ...
    if last_t > 0:
        ...
        print(
            f"[{_ts()}] ==> Done.  Advantage networks → {', '.join(p.name for p in paths)}"
        )
    else:
