...
_TPO_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.join(_TPO_DIR, "..", "evaluation"))
from match_runner import run_match  # noqa: E402
    ...
    for _ in range(epochs):
        ...
        for start in range(0, N, batch_size):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                adv_mean = (adv_clean * legal.float()).sum(
                    dim=-1, keepdim=True
                ) / n_legal
                u = (adv_clean - adv_mean).masked_fill(~legal, 0.0)
                u_std = (
                    (u.pow(2).sum(dim=-1, keepdim=True) / n_legal)
                    .sqrt()
                    .clamp(min=1e-4)
                )
                ...
                with torch.no_grad():
                    ...
                    kl = (
                        w * (pi * (log_pi_safe - log_q_dist)).sum(dim=-1)
                    ).sum()  # KL(p_θ ‖ q)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a .pt checkpoint to initialize the TPO network weights",
    )
    ...
    init_str = f"  init={args.init}\n" if args.init is not None else ""
    _term.write(
    ...
    if args.init is not None:
        hero_net = load_policy_net(args.init, device)
        hero_net.train()
        print(f"[{_ts()}]  Hero network initialized from: {args.init}")
    else:
    ...
    orch = deepcfr.Orchestrator(args.threads, seed=seed)
    ...
    for t in range(1, args.iters + 1):
        ...
        # -- Evaluation -------------------------------------------------------
        if run_eval and t % args.eval_interval == 0:
            print(
                f"[{_ts()}]  Evaluating hero vs villain ({args.eval_pairs:,} pairs)..."
            )
            mbb, ci95 = run_match(
                hero_net,
                villain_net,
            ...
            print(f"\n[{_ts()}]  Hero edge: {mbb:+.2f} +/- {ci95:.2f} mBB/hand")
