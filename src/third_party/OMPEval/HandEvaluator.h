...
class HandEvaluator {
  public:
    ...
    // Returns the rank of a hand as a 16-bit integer. Higher value is better.
    // Can also rank hands with less than 5 cards. A missing card is considered
    // the worst kicker, e.g. K < KQJT8 < A < AK < KKAQJ < AA < AA2 < AA4 <
    // AA432. Hand category can be extracted by dividing the value by 4096.
    // 1=highcard, 2=pair, etc.
  ...
  private:
    ...
    // Determines in how many rows the original lookup table is divided
    // (2^shift). More rows means slightly smaller lookup table but much bigger
    // offset table.
    static const unsigned PERF_HASH_ROW_SHIFT = 12;
    static const unsigned PERF_HASH_COLUMN_MASK =
        (1 << PERF_HASH_ROW_SHIFT) - 1;
