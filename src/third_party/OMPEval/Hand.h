...
struct Hand {
    ...
    // Create a Hand from a card. CardIdx is an integer between 0 and 51, so
    // that CARD = 4 * RANK + SUIT, where rank ranges from 0 (deuce) to 12 (ace)
    // and suit is from 0 (spade) to 3 (diamond).
    ...
    static Hand empty() {
        // Initializes suit counters to 3 so that the flush check bits gets set
        // by the 5th suited card.
        return EMPTY;
    }
    ...
    bool hasFlush() const {
        // Hand has a 4-bit counter for each suit. They start at 3 so the 4th
        // bit gets set when there is 5 or more cards of that suit. We can check
        // for flush by simply masking out the flush check bits.

#if OMP_SSE4 && !OMP_X64
        // On 32-bit systems it's faster to use PTEST.
        const __m128i fcmask =
            _mm_set_epi32(0, 0, FLUSH_CHECK_MASK32, 0); // sse2
    ...
    uint16_t flushKey() const {
        // Get the index of the flush check bit and use it to get the card mask
        // for that suit.
