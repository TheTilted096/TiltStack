...
│   │   ├── Node.cpp         # Hash-based game state decoding, regret matching, payouts
│   │   ├── Leduc.h          # LeducSolver class declaration
│   │   ├── Leduc.cpp        # Recursive CFR+ traversal with alternating updates
│   │   ├── BestResponse.h   # BestResponse class declaration
│   │   ├── BestResponse.cpp # Iterative best-response computation (20 passes)
│   │   └── bindings.cpp     # PyBind11 module exposing all classes to Python
│   └── pysrc/
│       ├── Leduc.py          # Training loop, card dealing/weighting, output, live plotting
│       ├── BestResponse.py   # Python wrapper for C++ BestResponse
│       └── leducsolver.pyd   # Compiled C++ extension (generated by build)
└── output/                   # Generated results (created at runtime)
    ├── leduc_results.txt     # Human-readable 528-node strategy
    ├── leduc_strategy.csv    # Machine-readable strategy (hash, check, bet, raise)
    ├── br_results.txt        # Best Response analysis with exploitability
    ├── exploitability.csv    # Iteration-by-iteration exploitability values
    └── exploitability.png    # Convergence graph
```
